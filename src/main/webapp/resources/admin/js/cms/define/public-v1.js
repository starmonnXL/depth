...
$(function() {
    ...
    $("#queryType,#query_type").submit(function (e) {
        ...
        var $var;
        if($type=="queryType"){
            $id = $(this).children("ul").find("li[class='active']").attr("id");
...
if($(".appointment-box").length>0){
	$(".appointment-box").slide({mainCell:".appointment-lists",autoPlay:true,effect:"topMarquee",interTime:100,trigger:"click"});
}
...
//新顾问列表（4人-轮播）start
	$('.counselor_man li').hover(function(){
				...
				if($(".counselor_man").length>0){
					$(".counselor_man").slide({titCell:"#con_dot ul",mainCell:"#con_main ul",autoPage:true,effect:"left",autoPlay:true,vis:4});
				}
