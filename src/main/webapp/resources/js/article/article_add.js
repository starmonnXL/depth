$(function () {
    ...
    $("#next").on("click",function () {
        var tag = $("#addArticle").form("validate");
        if(tag){
            ...
            $.post(
                ...
                function (response) {
                    if(response.status){
                        ...
                        $.messager.alert("提示信息", "添加失败" + (json.info));
                    }
                },"JSON"
            );
        }
...
function addFileIntro(index){
    ...
    if(rows != null && rows.length > 0){
        ...
        $("#addArticle input[name='picId']").val(ids);
        $("#addArticle input[name='picName']").val(fileNames);
        $("#addArticle #pic").attr("src",fileNames);
        /**保存文件信息到自己业务**/
        layer.close(index);
        return params;
    } else {
