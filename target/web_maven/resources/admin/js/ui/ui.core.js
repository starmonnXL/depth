...
var MUI = {
	...
	ajaxDone:function(json){
		...
		if(json.statusCode == MUI.statusCode.error) {
			...
			Msg.confirm('提示',json.message||"抱歉，您已登录超时!请重新登录！", function(r){
               if (r){
	...
	/**
	 * 打开一个简单的dialog的easyui窗口
	 * @param title 窗口的名称
	 * @param url 窗口请求的url地址
	 * @param id 窗口的ID信息
	 * @param width 窗口的宽度
	 * @param height 窗口的高度
	 */
	openSimpleDialog:function(title,url,id,width,height){
		$('#'+id).dialog({
		    title:title,
		    width: width,
		    height: height,
...
/**
 * 表格数据 选择多行，将url中{id}{name} 等占位符，
 * 替换成当前用户选择的行的数据
 * @param {} $this
 * @return {Boolean}
 */
function replaceSelectTmByDatagrid($this){
	var url=$this.attr("url");	
		var gid=$this.attr("datagrid");
		if(gid){
			var row=$("#"+gid).datagrid('getChecked');
			if(row.length > 0){
